...
export const parseVideoclipName = (videoclipPath: string, logger: Console) => {
    try {
        ...
        let flagsMapping = FLAGS_MAPPING[devType][version] as Record<number, [number, number]>;
        if (devType === 'hub' && version === 4) {
            flagsMapping = FLAGS_MAPPING[devType][2] as Record<number, [number, number]>;
        }
        ...
        console.error('Error parsing the filename', videoclipPath, e);
    }
}
