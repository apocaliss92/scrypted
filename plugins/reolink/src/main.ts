...
export const REOLINK_VIDEOCLIPS_INTERFACE = `${pluginId}:videoclips`;
export const videoclipsNativeId = 'reolinkVideoclips';
...
class ReolinkProvider extends RtspProvider {
    ...
    constructor(nativeId: string) {
        ...
        this.reportDevices().catch(this.console.error);
    }

    async reportDevices() {
        await sdk.deviceManager.onDeviceDiscovered(
            {
                name: 'Reolink Videoclips',
            ...
            try {
                if (webhook === 'videoclip') {
                    if (dev.storageSettings.values.ftp) {
                        ...
                        const { downloadPathWithHost } = await api.getVideoClipUrl(videoclipPath);
