...
import { VideoSearchResult, VideoSearchTime, VideoSearchType } from "./reolink-api";
import { getFolderPaths, parseVideoclipName, splitDateRangeByDay } from "./utils";
...
export default class ReolinkVideoclipssMixin extends SettingsMixinDeviceBase<any> implements Settings, VideoClips {
    ...
    private handleVideoError(videoPath: string): boolean {
        ...
        if (newCount > 2) {
            ...
            this.console.warn(`Video ${videoPath} blacklisted after ${newCount} failed attempts`);
            return true;
        }

        // this.console.debug(`Video ${videoPath} error count: ${newCount}/3`);
        return false;
    }
    ...
    async scanMemoryUsage(newMaxMemory?: number) {
        ...
        const { occupiedSpaceInGb, occupiedSpaceInGbNumber, freeMemory } = await this.calculateSize({
    ...
    async getClient() {
        return this.camera.getClientWithToken();
    }
    ...
    async getVideoClipThumbnail(thumbnailId: string, options?: VideoClipThumbnailOptions): Promise<MediaObject> {
        const logger = this.console;
        const { thumbnailUrl, videoclipUrl } = await this.getVideoclipParams(thumbnailId);
    ...
    async getVideoclipParams(videoclipId?: string) {
        const { ftp, ftpFolder } = this.storageSettings.values;
        const { thumbnailFolder } = getFolderPaths(this.id);
